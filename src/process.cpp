...
{ 
    ...
    // check if all required cpu statistics are available for utilization calculation
    if(cpu_util_stat_.size() == kValidDataSize_)
    {
        long total_time = cpu_util_stat_[kuTime_] + cpu_util_stat_[ksTime_];
        total_time = total_time + cpu_util_stat_[kCutTime_] + cpu_util_stat_[kCsTime_];

        float total_elapsed_time = LinuxParser::UpTime() -(cpu_util_stat_[kStartTime_] / sysconf(_SC_CLK_TCK));
        cpu_utilisation = 100 * ((total_time / sysconf(_SC_CLK_TCK)) / total_elapsed_time);
    }
